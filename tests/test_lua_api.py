    ...
    script.write_text(
        "\n".join(
            [
                ...
                '  topScores = { 4453, 4453, 4452, 4400 },',
                '  topCharacters = { { rank = 2, name = "TopHero", realmID = 707 } },',
                ...
                'local tied = assert(API:EstimateRank("cn", 4452.9, "all"))',
                'assert(tied.estimatedRank == 2 and tied.rankMin == 1 and tied.rankMax == 2)',
                'assert(tied.rankUncertainty == 1 and tied.isRoundedTie)',
                'local nextRank = assert(API:EstimateRank("cn", 4452, "all"))',
                'assert(nextRank.estimatedRank == 3 and not nextRank.isRoundedTie)',
                'assert(API:EstimateRank("cn", 3999, "all").isRoundedLeaderboardRank == nil)',
                'assert(API:GetCharacterTopRank("cn", "TopHero", 707) == 2)',
                'assert(API:GetCharacterTopRank("cn", "TopHero", 708) == nil)',
                'assert(API:GetCharacterTopRank("cn", "OtherHero", 707) == nil)',
                'UnitFullName = function(unit) assert(unit == "player"); return "TopHero", "TestRealm" end',
                'GetRealmID = function() return 707 end',
                'C_ChallengeMode = { GetOverallDungeonScore = function() return 4453 end }',
                'local exact = assert(API:EstimatePlayerRank("cn", "all"))',
                'assert(exact.estimatedRank == 2 and exact.rankMin == 2 and exact.rankMax == 2)',
                'assert(exact.isExactLeaderboardRank and not exact.isEstimate)',
                'GetRealmID = function() return 708 end',
                'local otherRealm = assert(API:EstimatePlayerRank("cn", "all"))',
                'assert(otherRealm.estimatedRank == 2 and not otherRealm.isExactLeaderboardRank)',
                'GetRealmID = function() return 707 end',
                'C_ChallengeMode.GetOverallDungeonScore = function() return 4452 end',
                'local stale = assert(API:EstimatePlayerRank("cn", "all"))',
                'assert(stale.estimatedRank == 2 and stale.isExactLeaderboardRank)',
                'local many = {}',
                'for i = 1, 1000 do many[i] = 4500 end',
                'for i = 1001, 1100 do many[i] = 3500 end',
                'API:GetRegionData("cn").topScores = many',
                'local grouped = assert(API:EstimateRank("cn", 3500, "all"))',
                'assert(grouped.rankMin == 1001 and grouped.rankMax == 1100)',
                'assert(grouped.estimatedRank == 1051 and grouped.rankUncertainty == 50)',
                ...
                'assert(API:GetCharacterTopRank("us", "TopHero", 707) == nil)',
